...
```text
tischkarte.example          website
device.tischkarte.example   device endpoint
```
...
1. Create a Cloudflare Pages project named `tischkarte` using **Upload assets**.
...
1. Create a bucket named exactly `device.tischkarte.example` in a suitable
   region.
...
Keep `TK_SYNC_PORT=80` and `TK_SYNC_INSECURE=y`.
