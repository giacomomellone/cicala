...
close to GPIO1. `CONFIG_CICALA_POWER_DIVIDER_NUM=2` and
`CONFIG_CICALA_POWER_DIVIDER_DEN=1` convert the ADC reading back to cell voltage.
...
next boot; `CONFIG_CICALA_POWER_WAKE_ON_USB` is disabled because the ESP32-S3 cannot
...
| ----------------------------- | -------------------------------------------------- | ------------------------------------ |
| Consecutive partial refreshes | minor artifacts at 193; visible ghosting before 64 | `CICALA_FULL_REFRESH_INTERVAL=16`    |
...
in steps while requesting partial refreshes. Set `CICALA_REFRESH_MIN_MV` above the
