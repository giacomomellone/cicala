...
- **Action:** a single tap on a hidden-for-now element. One vote per question per browser, deduplicated via `localStorage` (`cicala.voted` array). No undo UI; re-tapping is a no-op.
