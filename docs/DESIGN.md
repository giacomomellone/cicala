...
2. **The question persists at zero power.** E-paper keeps displaying it after everyone's attention has moved into the conversation — the device *is* the table card.
...
4. **It is finite and offline.** No feed, no notifications, no elsewhere to be. When the questions run out, they reshuffle — the device never asks for attention back.
...
- **Voting:** one anonymous "sparked a good conversation" action per question — design in [votes.md](votes.md), seam already in the DOM (`data-vote-slot`).
