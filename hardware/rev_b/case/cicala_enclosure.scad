...
function keeper_points(index) = [for(sign=[-1,1])
    [buttons_keeper_screw_x,buttons_cap_centres[index].y+sign*(buttons_cap_lengths[index]/2+buttons_keeper_screw_offsets[index][sign<0 ? 0 : 1])]];
...
module cap_slot(index, clearance=buttons_radial_clearance) {
    p=buttons_cap_centres[index]; len=buttons_cap_lengths[index];
...
module guide(index,clearance=buttons_radial_clearance) {
    p=buttons_cap_centres[index]; len=buttons_cap_lengths[index];
    difference() {
        rounded_block([99.25,p.y-len/2-3,buttons_guide_bottom_z],[10.6,len+6,3],.6);
        cap_slot(index,clearance);tab_pockets(index,clearance);flex_reserve();
        q=buttons_centres[index];
        block([q.x-3.3,q.y-3.3,6],[6.6,6.6,1]);
    }
}
...
module keeper_outline(index) {
    p=buttons_cap_centres[index]; len=buttons_cap_lengths[index];
    translate([97.35,p.y-len/2-1.9]) square([1.65,len+3.8]);
    for(sign=[-1,1]) {
        y=p.y+sign*(len/2+.35);
        hull() {
            translate([98.2,y]) circle(r=.85);
            translate([101.1,y]) circle(r=1.45);
        }
        hull() {
            translate([101.1,y]) circle(r=1.45);
            translate([buttons_keeper_screw_x,p.y+sign*(len/2+buttons_keeper_screw_offsets[index][sign<0 ? 0 : 1])]) circle(r=2.2);
        }
    }
}
module flex_reserve() {
    block([99.225,26.5,3.85],[2.15,13,3.8]);
}
module keeper(index=0, adjustment=keeper_adjustment) {
    difference() {
        ...
        p=buttons_cap_centres[index]; q=buttons_centres[index]; len=buttons_cap_lengths[index];
        for(sign=[-1,1]) {
            y=sign<0 ? p.y-len/2-.1-buttons_radial_clearance : p.y+len/2-buttons_strip_foot_inner_inset-buttons_radial_clearance;
            block([q.x-1.45-buttons_radial_clearance,y,keeper_bottom_z-eps],
...
module cap(index=0, travel=stroke, relief=cap_relief) {
    p=buttons_cap_centres[index]; q=buttons_centres[index]; len=buttons_cap_lengths[index]; x=p.x-buttons_cap_width/2;
    contact_z=pcb_z+pcb_thickness+.015+buttons_switch_height+relief;
    translate(side_shift+[0,0,-travel]) union() {
      difference() {
        union() {
            rounded_block([x,p.y-len/2,buttons_cap_body_bottom_z],
                [buttons_cap_width,len,case_height-buttons_face_recess-buttons_cap_body_bottom_z],buttons_face_radius);
            retention_tabs(index);
            for(sign=[-1,1]) {
                y=sign<0 ? p.y-len/2 : p.y+len/2-buttons_strip_foot_inner_inset;
                rounded_block([q.x-1.45,y,min(buttons_strip_foot_bottom_z,contact_z-.85)],
                    [2.9,buttons_strip_foot_inner_inset,buttons_cap_body_top_overlap-min(buttons_strip_foot_bottom_z,contact_z-.85)],.3);
            }
        }
        block([q.x-buttons_pad_width/2-buttons_pad_slot_clearance,p.y-len/2+buttons_strip_foot_inner_inset,5],
        ...
        for(sign=[-1,1]) {
            ...
            block([q.x-buttons_pad_width/2-buttons_pad_slot_clearance,y,contact_z-.025],
      ...
      translate([q.x,q.y,contact_z+buttons_compliant_pad_thickness])
...
module cap_pad(index=0,travel=stroke,relief=cap_relief) {
    p=buttons_cap_centres[index]; q=buttons_centres[index]; len=buttons_cap_lengths[index];
    block([q.x-buttons_pad_width/2,p.y-len/2+buttons_pad_end_inset,
        ...
        intersection() {
            ...
            union() {
                difference() {
                    outer();
                    difference() {
                        block([case_origin.x+case_wall,case_wall,-1],[case_width-2*case_wall,case_depth-2*case_wall,9.55]);
                        translate([0,0,-1.2]) bevel_volume();
                    }
                    block([case_origin.x-1,-1,-1],[case_width+2,case_depth+2,2.2]);
                }
...
module coupon(index=0) {
    p=buttons_cap_centres[index];
    intersection() {
        difference() {
            union() {
                guide(index,coupon_clearance);keeper_posts(index);
                block([99.25,p.y-14,8.0],[10.55,28,1.2]);
                block([109.2,p.y-14,5.9],[.6,28,3.3]);
            }
...
else if(part=="fit_caps_motion") intersection() { for(i=[0:1]) cap(i,stroke); union() { top_shell();display_frame_set();glass();lens();flex();components();cell();for(i=[0:1]) keeper(i); } }
...
else if(part=="fit_caps_lateral") intersection() { for(i=[0:1]) cap(i,stroke); union() {top_shell();display_frame_set();glass();lens();flex();components();for(i=[0:1]) keeper(i);} }
