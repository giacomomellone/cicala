...
class Copper:
    def __init__(self, geom, edge_clearance=0.3):
        self.geom = geom
        self.outline = Polygon(geom['outline'], geom['outline_holes'])
        self.edge_clearance = edge_clearance
        self.inner = self.outline.buffer(-edge_clearance)
        self.objects = []
        self.holes = []
        self.pads = []
        ...
        if recovery is not None:
            contact_layers = sorted({layer for pad in geom['pads'] if pad['ref'] == 'J4'
                                     for layer in pad['layers'] if layer in ('F.Cu', 'B.Cu')})
            self.keepouts.append({'layers':contact_layers,'tracks':False,'vias':False,
    ...
    def masks(self, net, width, via_size, grid, origin, shape):
        ...
        for diameter, is_via in ((width, False), (via_size, True)):
            ...
            raster(self.outline.buffer(-self.edge_clearance - radius - slack), inside, grid, origin)
