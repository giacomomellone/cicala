...
def audit(directory, geometry):
    from gerbonara.layers import LayerStack
    from gerbonara.utils import MM
    directory = Path(directory)
    ipc = check_ipc2581(directory / 'cicala_rev_a_ipc2581.zip')
    stack = LayerStack.open(directory / 'cicala_rev_a_gerbers.zip')
    ...
    for via in geometry['vias']:
        expected['pth'][key(via['at'][0] - 3, 3.5 - via['at'][1], via['drill'])] += 1
