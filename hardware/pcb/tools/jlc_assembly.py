...
def convert(bom_path, position_path, directory):
    ...
    with (directory / 'cicala_rev_a_jlc_bom.csv').open('w', newline='') as stream:
    ...
    with (directory / 'cicala_rev_a_jlc_cpl.csv').open('w', newline='') as stream:
        writer = csv.writer(stream)
