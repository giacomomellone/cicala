...
#ifdef CONFIG_CICALA_PANEL_LINES
constexpr uint8_t kMaxLines = CONFIG_CICALA_PANEL_LINES;
#else
constexpr uint8_t kMaxLines = 7;
#endif

#ifdef CONFIG_CICALA_MAX_QUESTION_BYTES
// Input byte count bounds the number of decomposed glyphs.
constexpr uint16_t kMaxGlyphs = CONFIG_CICALA_MAX_QUESTION_BYTES;
#else
