...
LOG_MODULE_REGISTER(cicala_soak, LOG_LEVEL_INF);
...
static void press(struct k_work *work)
{
...
#ifdef CONFIG_CICALA_DEBUG_SOAK_REBOOT
    if (presses > CONFIG_CICALA_DEBUG_SOAK_REBOOT_EVERY) {
    ...
    (void) k_work_reschedule(&press_work, K_MSEC(2 * CONFIG_CICALA_DEBUG_SOAK_INTERVAL_MS));

    const struct cicala_next_msg msg = {
...
static void on_render(const struct zbus_channel *chan)
{
    ...
    if (msg->result != 0) {
        ...
        LOG_INF("partial #%u of seq %u", cicala_panel_partial_count(), msg->seq);
    }

    rendered = true;

    (void) k_work_reschedule(&press_work, K_MSEC(CONFIG_CICALA_DEBUG_SOAK_INTERVAL_MS));
}

ZBUS_LISTENER_DEFINE(cicala_soak_obs, on_render);

/* 3 is the app thread's subscription and 4 is taken in the test suites. */
ZBUS_CHAN_ADD_OBS(chan_render, cicala_soak_obs, 5);
