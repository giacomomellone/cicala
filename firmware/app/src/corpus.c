...
size_t cicala_corpus_count(void)
{
    return ARRAY_SIZE(corpora);
}

const char *cicala_corpus_language(size_t index)
{
...
const uint8_t *cicala_corpus_data(size_t index, size_t *size)
{
    if (index >= ARRAY_SIZE(corpora)) {
        return NULL;
    }
...
int cicala_corpus_find(const char *code)
{
    if (code == NULL) {
        return -1;
    }
