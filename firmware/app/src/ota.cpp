...
LOG_MODULE_REGISTER(cicala_ota, LOG_LEVEL_INF);
...
{
    cicala::FirmwareRelease release = {};

    LOG_INF("checking %s for firmware newer than %s", CONFIG_CICALA_OTA_BASE_URL,
            APP_VERSION_STRING);
    ...
    const struct cicala_fetch_sink manifest_sink = {cicala_fetch_mem_write, &mem};

    int n = cicala_fetch(cicala_fetch_path_of(CONFIG_CICALA_OTA_BASE_URL "/firmware.json"),
                         &manifest_sink, CICALA_FETCH_TIMEOUT_MS);

    if (n < 0) {
        return CICALA_OTA_FAILED;
    }

    if (!cicala::firmware_parse(manifest_buf, (size_t) n, release)) {
        LOG_ERR("the firmware manifest did not parse");

        return CICALA_OTA_FAILED;
    }

    if (release.schema != cicala::kFirmwareSchema) {
        /* Unknown schemas may change field meaning. */
        LOG_ERR("firmware manifest schema %u, expected %u", release.schema,
                cicala::kFirmwareSchema);

        return CICALA_OTA_FAILED;
    }

    /* Reject signed manifests at or below the installed version. */
    if (cicala::version_compare(release.version, APP_VERSION_STRING) <= 0) {
        LOG_INF("running %s and the manifest offers %s — nothing to do", APP_VERSION_STRING,
                release.version);

        return CICALA_OTA_CURRENT;
    }
    ...
    const struct cicala_fetch_sink image_sink = {on_chunk, nullptr};

    n = cicala_fetch(cicala_fetch_path_of(release.url), &image_sink, CICALA_FETCH_IMAGE_TIMEOUT_MS);
    ...
    if (!cicala::ed25519_verify(release.sig, digest, sizeof(digest), CICALA_TRUSTED_KEY)) {
