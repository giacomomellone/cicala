...
static char ap_ssid[CICALA_SSID_MAX];
...
static struct cicala_scan_entry scan_results[CONFIG_CICALA_NET_SCAN_MAX];
static uint8_t scan_count;
static K_MUTEX_DEFINE(scan_lock);

/* What the form last submitted. */
static char pending_ssid[CICALA_SSID_MAX];
...
int cicala_portal_init(void)
{
    ...
    if (n >= 2) {
        (void) snprintf(ap_ssid, sizeof(ap_ssid), "%s-%02X%02X", CONFIG_CICALA_PORTAL_SSID_PREFIX,
                        id[n - 2], id[n - 1]);
    } else {
        (void) snprintf(ap_ssid, sizeof(ap_ssid), "%s", CONFIG_CICALA_PORTAL_SSID_PREFIX);
    }
...
uint32_t cicala_portal_window_remaining_s(void)
{
    ...
    const int64_t remaining = CONFIG_CICALA_PORTAL_WINDOW_MS - (k_uptime_get() - portal_started_ms);
...
bool cicala_portal_scan_start(void)
{
    struct wifi_scan_params params = {
        ...
        .max_bss_cnt = CONFIG_CICALA_NET_SCAN_MAX,
    };

    cicala_portal_scan_reset();
...
static void dns_thread(void *p1, void *p2, void *p3)
{
    ...
    static uint8_t query[CICALA_DNS_BUF_SIZE];
    static uint8_t reply[CICALA_DNS_BUF_SIZE];
    ...
    while (true) {
        ...
        const uint16_t len =
            cicala_dns_hijack(query, (uint16_t) n, AP_ADDR_HOST_ORDER, reply, sizeof(reply));
...
K_THREAD_DEFINE(cicala_dns_thread, DNS_STACK_SIZE, dns_thread, NULL, NULL, NULL, DNS_PRIORITY, 0,
                0);
...
static void first_ssid(void *arg, const char *ssid, size_t len)
{
    ...
    if (out[0] != '\0' || len == 0 || len >= CICALA_SSID_MAX) {
        return;
    }
                         ...
                         void *user_data)
{
    ...
    k_mutex_lock(&scan_lock, K_FOREVER);
    const int n = cicala_page_setup((char *) page_buf, sizeof(page_buf), scan_results, scan_count,
                                    cicala_language());
    k_mutex_unlock(&scan_lock);
                          ...
                          struct http_response_ctx *response, void *user_data)
{
    ...
    const bool started = cicala_portal_scan_start();
    const int n = cicala_page_notice(
                          ...
                          struct http_response_ctx *response, void *user_data)
{
    ...
    cicala_portal_copy_connection_error(last_error, sizeof(last_error));
    cicala_portal_copy_sync_result(last_sync, sizeof(last_sync));

    const int n = cicala_page_status(
        (char *) page_buf, sizeof(page_buf), ap_ssid, pending_ssid[0] != '\0' ? pending_ssid : NULL,
        station_connected, station_ip, last_error, last_sync, cicala_portal_window_remaining_s());
                        ...
                        void *user_data)
{
    ...
    const int ssid_len =
        cicala_form_field(form_body, (uint16_t) form_len, "ssid", ssid, sizeof(ssid));
    const int psk_len = cicala_form_field(form_body, (uint16_t) form_len, "psk", psk, sizeof(psk));

    form_len = 0;

    if (ssid_len <= 0) {
        const int n = cicala_page_notice((char *) page_buf, sizeof(page_buf), "No network selected",
    ...
    const int n = cicala_page_saved((char *) page_buf, sizeof(page_buf), ssid);

    /* The page goes out first and the join is left to `net`. */
    cicala_net_notify_credentials();
                            ...
                            struct http_response_ctx *response, void *user_data)
{
    ...
    char lang[CICALA_LANGUAGE_LEN] = {0};
    const int lang_len =
        cicala_form_field(form_body, (uint16_t) form_len, "lang", lang, sizeof(lang));

    form_len = 0;

    if (lang_len != 2 || cicala_language_set(lang) != 0) {
        const int n = cicala_page_notice((char *) page_buf, sizeof(page_buf), "Language not saved",
                                         "That language is not available in this device image.");

        return send_page("/language", n, response);
    }

    const struct cicala_corpus_msg msg = {.language = {lang[0], lang[1], '\0', '\0'}};

    /* `app` reopens the store. */
    (void) zbus_chan_pub(&chan_corpus, &msg, K_MSEC(100));

    const int n = cicala_page_saved((char *) page_buf, sizeof(page_buf), NULL);
                          ...
                          struct http_response_ctx *response, void *user_data)
{
    ...
    const int confirm_len =
        cicala_form_field(form_body, (uint16_t) form_len, "confirm", confirm, sizeof(confirm));

    form_len = 0;

    if (confirm_len != 6 || strcmp(confirm, "forget") != 0) {
        const int n = cicala_page_forget_confirm((char *) page_buf, sizeof(page_buf));

        return send_page("/forget", n, response);
    }

    const int err = cicala_portal_forget_credentials();
    const int n = cicala_page_notice(
                        ...
                        void *user_data)
{
    ...
    cicala_net_notify_sync();

    const int n = cicala_page_notice((char *) page_buf, sizeof(page_buf), "Update requested",
...
HTTP_SERVICE_DEFINE(cicala_portal, NULL, &http_port, CONFIG_HTTP_SERVER_MAX_CLIENTS, 4, NULL,
                    &catchall_detail.common, NULL);

HTTP_RESOURCE_DEFINE(setup_resource, cicala_portal, "/", &setup_detail);
HTTP_RESOURCE_DEFINE(status_resource, cicala_portal, "/status", &status_detail);
HTTP_RESOURCE_DEFINE(rescan_resource, cicala_portal, "/scan", &rescan_detail);
HTTP_RESOURCE_DEFINE(save_resource, cicala_portal, "/save", &save_detail);
HTTP_RESOURCE_DEFINE(language_resource, cicala_portal, "/language", &language_detail);
HTTP_RESOURCE_DEFINE(sync_resource, cicala_portal, "/sync", &sync_detail);
HTTP_RESOURCE_DEFINE(forget_resource, cicala_portal, "/forget", &forget_detail);
