...
#define RUN_WAIT K_MSEC(10 * CONFIG_CICALA_DEBUG_SOAK_INTERVAL_MS + 1000)
...
static void observe(const struct zbus_channel *chan)
{
    if (chan == &chan_question) {
        const struct cicala_question_msg *msg = zbus_chan_const_msg(chan);
...
ZTEST_SUITE(cicala_soak, NULL, suite_setup, NULL, NULL, NULL);

ZTEST(cicala_soak, test_the_chain_runs_hands_off)
{
    k_sleep(RUN_WAIT);
